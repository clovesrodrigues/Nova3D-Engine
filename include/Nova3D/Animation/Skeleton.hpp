...
class BoneHierarchy { public: std::int32_t addBone(Bone bone); std::int32_t findBone(const std::string& name) const; const std::vector<Bone>& bones() const { return m_bones; } std::vector<std::int32_t> childrenOf(std::int32_t parent) const; private: std::vector<Bone> m_bones; };
