...
class CameraTargetSceneNode : public SceneNode { public: explicit CameraTargetSceneNode(math::Vector3 offset={0,0,-1}):m_offset(offset){} void setLinkedNode(const std::shared_ptr<ISceneNode>& node){ m_linkedNode=node; } void setFollowOffset(const math::Vector3& offset){ m_offset=offset; } void setFollowLinkedNode(bool enabled){ m_followLinkedNode=enabled; } void onUpdate(float) override; private: std::weak_ptr<ISceneNode> m_linkedNode; math::Vector3 m_offset{0,0,-1}; bool m_followLinkedNode=true; };
class CameraSceneNode : public SceneNode { public: void setPerspective(float fovDeg, float aspect, float nearPlane, float farPlane); void setOrthographic(float left, float right, float bottom, float top, float nearPlane, float farPlane); void lookAt(const math::Vector3& target, const math::Vector3& up = {0,1,0}); void onUpdate(float dt) override; void setTargetNode(const std::shared_ptr<CameraTargetSceneNode>& target){ m_targetNode=target; } std::shared_ptr<CameraTargetSceneNode> targetNode() const { return m_targetNode.lock(); } void setCameraTargetLocked(bool locked){ m_lockCameraAndTarget = locked; } void setViewport(const graphics::Viewport& viewport){m_viewport=viewport;} const graphics::Viewport& viewport() const { return m_viewport; } void setRenderTarget(const std::shared_ptr<graphics::IRenderTarget>& target){m_target=target;} std::shared_ptr<graphics::IRenderTarget> renderTarget() const { return m_target; } const math::Matrix4& viewMatrix() const { return m_view; } const math::Matrix4& projectionMatrix() const { return m_proj; } math::Matrix4 viewProjectionMatrix() const { return m_proj*m_view; } math::Frustum frustum() const { return math::Frustum::fromViewProjection(viewProjectionMatrix()); } private: std::weak_ptr<CameraTargetSceneNode> m_targetNode; bool m_lockCameraAndTarget=false; math::Vector3 m_lockOffset{0,0,-1}; math::Matrix4 m_view=math::Matrix4::identity(); math::Matrix4 m_proj=math::Matrix4::identity(); graphics::Viewport m_viewport{0,0,1280,720}; std::shared_ptr<graphics::IRenderTarget> m_target; };
