...
void MeshSceneNode::onRegister(SceneManager& mgr){ if(!m_mesh) return; for(size_t i=0;i<m_mesh->meshBufferCount();++i) mgr.registerVisible(m_mesh->meshBuffer(i), worldMatrix()); }
...
void SceneManager::registerVisible(const std::shared_ptr<IMeshBuffer>& buffer,const math::Matrix4& world){ if (auto m=buffer->material(); m && m->isTransparent()) m_transparentQueue.push_back({buffer,world}); else m_opaqueQueue.push_back({buffer,world}); }
...
void SceneManager::gather(){ if(!m_camera) return; m_opaqueQueue.clear(); m_transparentQueue.clear(); m_lighting.directionalLights.clear(); m_lighting.pointLights.clear(); auto fr=m_camera->frustum(); std::vector<std::shared_ptr<ISceneNode>> stack{m_root}; while(!stack.empty()){ auto n=stack.back(); stack.pop_back(); if(!n->visible()) continue; if(fr.intersects(n->worldBounds())) n->onRegister(*this); for(auto& c:n->children()) stack.push_back(c);} }
...
std::shared_ptr<IMesh> createSimpleTriangleMesh(){ auto mb=std::make_shared<MeshBuffer>(); mb->v={{{0,0.6F,0},{1,0,0}},{{-0.6F,-0.6F,0},{0,1,0}},{{0.6F,-0.6F,0},{0,0,1}}}; mb->i={0,1,2}; mb->box={{-0.6F,-0.6F,0},{0.6F,0.6F,0}}; auto m=std::make_shared<Mesh>(); m->bufs.push_back(mb); m->box=mb->box; return m; }
}
