...
std::int32_t BoneHierarchy::findBone(const std::string& name) const { for(std::int32_t i=0;i<static_cast<std::int32_t>(m_bones.size());++i) if(m_bones[i].name==name) return i; return -1; }
