...
math::Vector4 NGUIStyleResolver::color(const NGUITheme& theme,const NGUIStyleSheet& sheet,const std::string& styleClass,const std::string& key,const math::Vector4& fallback) const { if(auto c=sheet.classColors.find(styleClass); c!=sheet.classColors.end()){ if(auto v=c->second.find(key); v!=c->second.end()) return v->second;} if(auto t=theme.colors.find(key); t!=theme.colors.end()) return t->second; return fallback; }
...
NDialogResult NFileDialog::open(const std::string& title,const std::string& filter,bool){ core::Logger::info("[GUI] FileDialog open: "+title+" filter="+filter); return {true,{"assets/models/cube.obj"}}; }
NDialogResult NFileDialog::save(const std::string& title,const std::string& filter){ core::Logger::info("[GUI] FileDialog save: "+title+" filter="+filter); return {true,{"sandbox_saved.layout.json"}}; }
